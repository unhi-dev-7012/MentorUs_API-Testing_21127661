...
RequestObject req = findTestObject('API_Update Profile', [('userId') : GlobalVariable.myId, ('name') : name
...
String formattedBirthDate = birthDate + 'T00:00:00.000+00:00'
if(birthDate == '')
{
...
if (expected.intValue() == 200)
{
	...
	WS.verifyElementPropertyValue(res, 'data.id', GlobalVariable.myId, FailureHandling.CONTINUE_ON_FAILURE)
	WS.verifyElementPropertyValue(res, 'data.name', name, FailureHandling.CONTINUE_ON_FAILURE)
	WS.verifyElementPropertyValue(res, 'data.birthDate', formattedBirthDate, FailureHandling.CONTINUE_ON_FAILURE)
