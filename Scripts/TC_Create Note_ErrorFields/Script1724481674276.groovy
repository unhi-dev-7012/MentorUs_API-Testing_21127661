...
if (expected.intValue() != 403)
{
	def errorField = WS.getElementPropertyValue(res, 'properties.errors[0].field', FailureHandling.CONTINUE_ON_FAILURE)
	
	if (title == '')
	{
		assert errorField == 'title' : "Expected error message 'title', but got '${errorField}'"
	}
	else if (content == '')
	{
		assert errorField== 'content' : "Expected error message 'content', but got '${errorField}'"
	}
